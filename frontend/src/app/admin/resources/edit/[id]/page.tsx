...
export default function EditResourcePage() {
    ...
    useEffect(() => {
        const fetchDetailsAndTags = async () => {
            if (!id) return;
            try {
                const [detailsRes, listRes] = await Promise.all([
                    api.get(`/resources/${id}`),
                    api.get("/resources", { params: { limit: 50 } })
                ]);
                
                const resource = detailsRes.data.data;
                if (resource) {
                    setFormData({
                        ...
                        minTier: resource.minTier || "FREE",
                    });

                    // Determine selected tiers based on hierarchy
                    const tiers = ["FREE", "UBUNTU", "IMANI", "KIONGOZI"];
                    const index = tiers.indexOf(resource.minTier || "FREE");
                    setSelectedTiers(tiers.slice(0, index + 1));
                }

                const tags = new Set<string>(["General", "Strategic", "Community", "Leadership"]);
                listRes.data.data?.forEach((r: any) => {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-50">
                <div className="text-center">
                    <Loader2 className="animate-spin h-10 w-10 text-tatt-lime mx-auto mb-4" />
        ...
        <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-20">
            ...
            <div className="max-w-5xl mx-auto w-full p-4 lg:p-12">
                <div className="mb-10 animate-in fade-in slide-in-from-top-4 duration-500 flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-4xl font-black tracking-tighter text-slate-900 mb-1 uppercase italic">Edit Resource</h1>
                        <p className="text-slate-500 text-sm font-medium">Modify designated knowledge assets, guides, and premium documentation.</p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-end gap-6 py-10 border-t border-slate-200">
                        ...
                        <button 
                            ...
                            type="submit"
                        >
                            {isSubmitting ? <Loader2 size={20} className="animate-spin text-tatt-lime" /> : <CheckCircle2 size={20} />}
                            Save Changes
                        </button>
                    </div>
                </form>
